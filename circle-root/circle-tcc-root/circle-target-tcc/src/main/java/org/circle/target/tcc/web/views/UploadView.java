...
public class UploadView implements Serializable {
	...
	private static final String CONVERSATION_NAME = "EXTRACTION";

	private UploadedFile uploadedFile;
	private Document document;

	@EJB DocumentValidationService documentValidationService;
	...
	public String generateExtraction() {
		try {
			...
			FacesContextUtil.sendMessageToContext(FacesMessage.SEVERITY_WARN, SUMARY_EMPTY_STREAM, DETAIL_EMPTY_STREAM, ID_COMPONENT_MESSAGE);
			return NAV_ERROR_STREAM;
			
		} catch (FileOversizedException e) {
			FacesContextUtil.sendMessageToContext(FacesMessage.SEVERITY_WARN, SUMARY_FILE_OVERSIZE, DETAIL_FILE_OVERSIZE, ID_COMPONENT_MESSAGE);
			return NAV_ERROR_STREAM;
			
		} catch (ExtensionNotMappedException e) {
			FacesContextUtil.sendMessageToContext(FacesMessage.SEVERITY_WARN, SUMARY_EXT_NOT_MAPPED, DETAIL_EXT_NOT_MAPPED, ID_COMPONENT_MESSAGE);
			return NAV_ERROR_STREAM;
			
		} catch (ExtensionEmptyException e) {
			FacesContextUtil.sendMessageToContext(FacesMessage.SEVERITY_WARN, SUMARY_EXT_EMPTY, DETAIL_EXT_EMPTY, ID_COMPONENT_MESSAGE);
			return NAV_ERROR_STREAM;
			
		} catch (IOException e) {
			FacesContextUtil.sendMessageToContext(FacesMessage.SEVERITY_ERROR, SUMARY_ERROR_IO, DETAIL_ERROR_IO, ID_COMPONENT_MESSAGE);
			return NAV_ERROR_STREAM;
		}
	}
